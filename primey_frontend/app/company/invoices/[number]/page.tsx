...
export default function CompanyInvoicePage() {
  ...
  const [method, setMethod] = useState<PaymentMethod>("TAMARA")
  ...
  async function loadInvoice(showErrorToast = true) {
    if (!invoiceNumber) {
      setInvoice(null)
      setLoading(false)
      return
    ...
    try {
      ...
      if (!res.ok) {
        if (showErrorToast) {
          toast.error(data.error || data.message || "Invoice not found")
        }
        setInvoice(null)
        return
      }
  ...
  async function handleBankTransferPayment() {
    toast.info("Bank transfer receipt upload will be added later")
  }

  async function handleCreditCardPayment() {
    toast.info("Credit card gateway will be connected later")
  }
  ...
  async function handleTamaraPayment() {
    ...
    const res = await fetch(
      `${API}/company/payments/tamara/create-checkout/`,
      {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    if (!res.ok) {
      throw new Error(data.error || data.message || "Failed to create Tamara checkout")
    }
    ...
    toast.success("Redirecting to Tamara...")
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl space-y-6 p-6">
        <div className="no-print flex flex-col gap-4 rounded-3xl border bg-background p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="grid gap-6 xl:grid-cols-12">
            ...
            <CardContent className="print-body space-y-6 p-6">
              ...
              <div className="print-total-grid grid gap-4 md:grid-cols-[1fr_180px] md:items-stretch md:justify-end">
                ...
                <div className="print-qr-inline flex flex-col items-center justify-center rounded-2xl border bg-white p-4">
                  <div className="no-print mb-2 flex items-center gap-2 text-sm font-medium text-muted-foreground">
          ...
          <div className="no-print space-y-6 xl:col-span-4">
            {invoice.status === "PAID" ? (
                  ...
                  <CardContent className="space-y-3">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium">Tamara</span>
                      </div>
                  ...
                  <CardContent className="space-y-4 p-6">
                    ...
                    {method === "TAMARA" && (
                      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground">
                        You will be redirected to Tamara to complete the payment securely.
                      </div>
                    )}

                    {method === "CREDIT_CARD" && (
                      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground">
                        Credit card gateway will be connected in the next phase.
                      </div>
                    )}

                    {method === "BANK_TRANSFER" && (
                      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground">
                        Bank transfer receipt upload will be added in the next phase.
                      </div>
                    )}
